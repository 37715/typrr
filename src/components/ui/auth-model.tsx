    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            <div className="overflow-auto max-h-[90vh] rounded-2xl border border-white/10 bg-white/90 shadow-2xl backdrop-blur-xl dark:border-white/10 dark:bg-neutral-900/90">
                ...
                <div className="flex gap-1 rounded-full bg-zinc-200 p-1 dark:bg-white/5">
                  <button
                    ...
                    className={`rounded-full px-3 py-1.5 text-sm font-medium transition ${
                      mode === 'login' ? 'bg-white text-zinc-900 shadow-sm dark:bg-white/90' : 'text-zinc-700 hover:text-zinc-900 dark:text-white/80 dark:hover:text-white'
                    }`}
                  >
                    sign in
                  </button>
                  <button
                    ...
                    className={`rounded-full px-3 py-1.5 text-sm font-medium transition ${
                      mode === 'signup' ? 'bg-white text-zinc-900 shadow-sm dark:bg-white/90' : 'text-zinc-700 hover:text-zinc-900 dark:text-white/80 dark:hover:text-white'
                    }`}
                  >
                    create account
                  </button>
...
function SocialButton({ label, onClick, loading }: { label: string; onClick?: () => void; loading?: boolean }) {
  return (
    <button
      ...
      onClick={onClick}
      disabled={loading}
      className="group inline-flex items-center justify-center gap-2 rounded-xl border border-zinc-300 bg-white px-3 py-2 text-sm font-medium text-zinc-900 shadow-sm transition hover:bg-zinc-100 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60 dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:bg-white/10"
    >
      <span>{label}</span>
    </button>
  );
